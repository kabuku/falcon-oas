...
from .exceptions import MissingRequestBody, RequestBodyError, ValidationError
...
class RequestBodyUnmarshaler(object):
    ...
    def unmarshal(self, get_value, media_type, request_body_spec_dict):
        ...
        logger.info("Media type: %s, request body: %r", media_type, value)
        ...
        try:
            ...
            logger.warning(
                "Failed to unmarshal request body %r with %r",
                value,
                media_type_spec_dict,
                exc_info=True,
            )
